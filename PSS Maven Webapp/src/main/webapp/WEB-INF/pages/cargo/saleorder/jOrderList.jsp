...
<table id="ec_table" class="tableRegion" width="98%" >
	...
	<tr class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'" >
		...
		<td>
			<c:if test="${o.state == 1}"><a href = "${ctx }/cargo/saleorder/start.action?id=${o.id}"><font color="blue">${orderState['1'] }</font></a></c:if>
			<c:if test="${o.state == 2}"><font color="gray">${orderState['2'] }</font></c:if>
			<c:if test="${o.state == 3}"><font color="green">${orderState['3'] }</font></c:if>
			<c:if test="${o.state == 4}"><a href = "${ctx }/cargo/saleorder/start.action?id=${o.id}"><font color="red">重新审核</font></a></c:if>
		</td>
		<td>
			<c:if test="${o.state == 1  || o.state == 4}">
				<a href="${ctx }/cargo/saleorder/toaddproduct.action?orderNo=${o.orderNo}"><font color="blue">货物</font></a>
			</c:if>
